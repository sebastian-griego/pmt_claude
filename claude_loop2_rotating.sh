...
ITERATION_FILE="..iteration_count2"
...
while true; do
    [ -f STOP2 ] && { echo "STOP2 detected"; exit 0; }
    
    # Check if we need to rotate PROGRESS2.md
    ITERATION=$(cat "$ITERATION_FILE")
    ITERATION=$((ITERATION + 1))
    echo "$ITERATION" > "$ITERATION_FILE"
    
    if [ $((ITERATION % 10)) -eq 0 ]; then
        echo "[rotation] Archiving PROGRESS2.md at iteration $ITERATION"
        cp PROGRESS2.md "logs/PROGRESS_archived_$(date +%Y%m%d_%H%M%S).md"
        # Keep only last 100 lines as context
        tail -100 PROGRESS2.md > PROGRESS_tmp.md
        mv PROGRESS_tmp.md PROGRESS2.md
    fi
    
    TS="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    PROG_TAIL="$(tail -c 40000 PROGRESS2.md 2>/dev/null || true)"
