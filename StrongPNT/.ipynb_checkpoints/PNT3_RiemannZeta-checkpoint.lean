...
lemma norm_inv (z : ℂ) (hz : z ≠ 0) : ‖z⁻¹‖ = ‖z‖⁻¹ := by
  simp [Complex.norm_inv]

/-- Real part of quotient formula -/
lemma re_div (w z : ℂ) (hz : z ≠ 0) : (w / z).re = (w.re * z.re + w.im * z.im) / ‖z‖^2 := by
  simp [Complex.div_re]

/-- Imaginary part of quotient formula -/
lemma im_div (w z : ℂ) (hz : z ≠ 0) : (w / z).im = (w.im * z.re - w.re * z.im) / ‖z‖^2 := by
  simp [Complex.div_im]
...
lemma prod_positive :
  ...
  sorry  -- Requires showing multipliability and using tprod_pos
